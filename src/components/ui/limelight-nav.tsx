...
export const LimelightNav = ({
  ...
  return (
    <nav className={`relative inline-flex items-center h-16 rounded-lg bg-transparent text-white border-transparent px-2 ${className}`}>
      {items.map(({ id, icon, label, onClick }, index) => (
          <a
            key={id}
            ref={el => (navItemRefs.current[index] = el)}
            className={`relative z-20 flex h-full cursor-pointer items-center justify-center transition-all duration-200 ease-in-out hover:bg-white/10 rounded-lg ${iconContainerClassName}`}
            onClick={() => handleItemClick(index, onClick)}
            aria-label={label}
          >
            {cloneElement(icon, {
              className: `w-6 h-6 transition-all duration-200 ease-in-out ${
                activeIndex === index ? 'opacity-100 scale-110' : 'opacity-70 hover:opacity-90'
      ...
      <div 
        ref={limelightRef}
        className={`absolute top-0 z-10 w-11 h-[5px] rounded-full bg-blue-500 shadow-[0_0_20px_rgba(59,130,246,0.8)] ${
          isReady ? 'transition-[left] duration-400 ease-in-out' : ''
        } ${limelightClassName}`}
        style={{ left: '-999px' }}
      >
        <div className="absolute left-[-30%] top-[5px] w-[160%] h-14 [clip-path:polygon(5%_100%,25%_0,75%_0,95%_100%)] bg-gradient-to-b from-blue-500/30 to-transparent pointer-events-none" />
      </div>
    </nav>
  );
};